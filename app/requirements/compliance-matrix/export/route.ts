...
export async function GET() {
  ...
  const sortedInstances = [...(instances ?? [])].sort((a: any, b: any) => {
    const tplA = a.requirement_templates as any;
    const tplB = b.requirement_templates as any;
    const catA = tplA?.requirement_categories?.id ?? 0;
    const catB = tplB?.requirement_categories?.id ?? 0;
    if (catA !== catB) return catA - catB;
    return (tplA?.id ?? 0) - (tplB?.id ?? 0);
  });
  ...
  for (const inst of sortedInstances) {
    const tpl = inst.requirement_templates as any;
    headers.push(tpl?.name ?? `Instance #${inst.id}`);
  }
    ...
    for (const inst of sortedInstances) {
      ...
      if (!sub) {
        ...
        if (sub.status === "submitted") {
          const step = sub.current_step < 4 ? STEP_LABELS[sub.current_step] : "";
  ...
  const ay = activeSemester.academic_years as any;
  const semesterLabel = `${ay?.label ?? ""} ${activeSemester.name}`;
