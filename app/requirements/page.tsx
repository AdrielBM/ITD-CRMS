            ...
            {canWrite && (
              <form action={createInstance} style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
                ...
                <select name="semester_id" required className="input" style={{ flex: 1, minWidth: 160 }}>
                  ...
                  {(semesters ?? []).map((s: any) => (<option key={s.id} value={s.id}>{s.academic_years?.label} · {s.name}</option>))}
                </select>
            ...
            {canWrite && (
              <form action={createAssignment} style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
                <select name="requirement_instance_id" required className="input" style={{ flex: 2, minWidth: 200 }}>
                  ...
                  {(instances ?? []).map((i: any) => (<option key={i.id} value={i.id}>{i.requirement_templates?.name} · {i.semesters?.academic_years?.label} {i.semesters?.name}</option>))}
                </select>
                ...
                <select name="assignee_id" required className="input" style={{ flex: 2, minWidth: 160 }}>
                  <optgroup label="Faculty">
                    {(faculty ?? []).map((f: any) => (<option key={`f-${f.id}`} value={f.id}>{f.full_name}</option>))}
                  </optgroup>
                  <optgroup label="Organizations">
                    {(organizations ?? []).map((o: any) => (<option key={`o-${o.id}`} value={o.id}>{o.name}</option>))}
                  </optgroup>
                </select>
