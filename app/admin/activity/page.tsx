...
export default async function ActivityLogPage({ searchParams }: { searchParams?: { page?: string } }) {
  ...
  const reviewerIds = [...new Set((approvals ?? []).map((a: any) => a.reviewer_id))];
  const facultyIds = [...new Set((approvals ?? []).map((a: any) => a.submissions?.assignments?.faculty_id).filter(Boolean))];
