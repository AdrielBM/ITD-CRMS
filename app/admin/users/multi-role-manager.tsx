...
function RoleBadge({ roleName, userId, optimistic, pending }: { roleName: string; userId: string; optimistic?: boolean; pending?: boolean }) {
  return (
...
export default function MultiRoleManager({ userId, assignedRoles, allRoles }: { userId: string; assignedRoles: { id: number; name: string }[]; allRoles: { id: number; name: string }[] }) {
      ...
      {unassigned.length > 0 && (
        <form action={addUserRole} style={{ display: "inline" }}>
          ...
          <select name="role_name" defaultValue=""
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => { if (e.target.value) e.target.form!.submit(); }}
