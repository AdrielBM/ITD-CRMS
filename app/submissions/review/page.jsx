...
const WORKFLOW_STEPS = [
  { step: 0, role: "Coordinator", label: "Coordinator Review" },
  { step: 1, role: "Records", label: "Records Verification" },
  { step: 2, role: "Secretary", label: "Secretary Compliance" },
  { step: 3, role: "Chair", label: "Chair Final Approval" },
];
            ...
            return (
              <div key={sub.id} className="card">
                ...
                <form action={async (formData) => {
                  ...
                  if (action === "approve") return approveSubmission(formData);
                  if (action === "reject") return rejectSubmission(formData);
...
async function SubFiles({ submissionId }) {
  const supabase = await createClient();
