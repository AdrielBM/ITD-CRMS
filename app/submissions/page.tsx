...
export default async function SubmissionsPage() {
  ...
  let query: any = supabase.from("assignments").select(`
    id,
  ...
  return (
    <AppShell fullName={profile?.full_name} email={user.email} role={role} roles={roles} currentPath="/submissions">
      ...
      <div className="page-body">
        {/* Status legend */}
        <div style={{ marginBottom: 16, display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center", fontSize: 13, padding: "12px 16px", background: "#f9fafb", borderRadius: 8 }}>
          <span style={{ fontWeight: 600, color: "#374151" }}>Status:</span>
          <span><span className="badge badge-gray" style={{ marginRight: 4 }}>Pending</span> Not yet submitted</span>
          <span><span className="badge badge-blue" style={{ marginRight: 4 }}>Under Review</span> Awaiting approval</span>
          <span><span className="badge badge-red" style={{ marginRight: 4 }}>Returned</span> Needs revision</span>
          <span><span className="badge badge-green" style={{ marginRight: 4 }}>Approved</span> Fully approved</span>
        </div>
        ...
        {isFaculty && (
          <div className="card" style={{ marginBottom: 16, background: "#f0fdf4", border: "1px solid #bbf7d0" }}>
            <p style={{ margin: 0, fontSize: 14, color: "#166534" }}>
              <strong>How to submit:</strong> Find a requirement below, upload your file(s), add optional notes, then click <strong>&ldquo;Submit for Review&rdquo;</strong>. Your submission will go through a review pipeline (Coordinator → Records → Secretary → Chair). Track status here and in your notifications.
            </p>
          </div>
        )}
            ...
            return (
              <div key={a.id} className="card">
                ...
                {sub?.status === "completed" && (
                  <div style={{ marginTop: 12, fontSize: 13, color: "#16a34a", fontWeight: 500 }}>✓ Approved — passed all review steps</div>
                )}
...
async function SubFilesInline({ submissionId }: { submissionId: number }) {
  const supabase = await createClient();
