          ...
          <section>
            ...
            <div className="table-wrap">
              <table>
                ...
                <tbody>
                  {(programs ?? []).map((p: any) => {
                    const progCoords = (coordinators ?? []).filter((c: any) => c.program_id === p.id);
                    const unassigned = (coordinators ?? []).filter((c: any) => c.program_id === null);
                    return (
                      <tr key={p.id}>
                            ...
                            {canWritePrograms && (
                              <form action={assignProgramCoordinator} style={{ display: "inline-flex", gap: 4 }}>
                                ...
                                <select name="user_id" required className="input" style={{ padding: "2px 6px", fontSize: 12, maxWidth: 180 }}>
                                  ...
                                  {(coordinatorUsers ?? [])
                                    .filter((u: any) => !progCoords.some((pc: any) => pc.user_id === u.id) && !unassigned.some((uc: any) => uc.user_id === u.id))
            ...
            {canWritePrograms && (
              <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid #e5e7eb" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center" }}>
                  {(coordinators ?? []).filter((c: any) => c.program_id === null).map((pc: any) => (
                    <span key={pc.id} className="badge badge-blue" style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                      {pc.user?.full_name ?? pc.user?.email}
                      <form action={removeProgramCoordinator} style={{ display: "inline" }}>
                        <input type="hidden" name="id" value={pc.id} />
                  ...
                  <form action={assignProgramCoordinator} style={{ display: "inline-flex", gap: 4 }}>
                    ...
                    <select name="user_id" required className="input" style={{ padding: "2px 6px", fontSize: 12, maxWidth: 180 }}>
                      ...
                      {(coordinatorUsers ?? [])
                        .filter((u: any) => !(coordinators ?? []).some((c: any) => c.user_id === u.id))
                        .map((u: any) => <option key={u.id} value={u.id}>{u.full_name ?? u.email}</option>)}
                    </select>
                    <button className="btn btn-primary btn-sm" style={{ padding: "2px 10px", fontSize: 12 }}>Assign</button>
                  </form>
            ...
            {canWritePrograms && (
              <form action={createSemester} style={{ display: "flex", gap: 8, marginTop: 12 }}>
                <select name="academic_year_id" required className="input" style={{ flex: 1 }}>
                  ...
                  {(academicYears ?? []).map((ay: any) => (<option key={ay.id} value={ay.id}>{ay.label}</option>))}
                </select>
