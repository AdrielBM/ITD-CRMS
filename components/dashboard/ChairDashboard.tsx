...
import AcademicStructureOverview from "./AcademicStructureOverview";

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type BarDataItem = any;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type PieDataItem = any;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type RecentUser = any;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type CategoryDatum = any;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type Program = any;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
const ChairDashboard: FC<Props> = ({
  ...
  const barData = (categoryData ?? []).map((c: any) => ({
    name: c.name,
  ...
  const pieData = [
    { name: "Approved", value: barData.reduce((s: number, c: any) => s + c.Approved, 0), color: "#16a34a" },
    { name: "Submitted", value: barData.reduce((s: number, c: any) => s + c.Submitted, 0), color: "#ea580c" },
    { name: "Returned", value: barData.reduce((s: number, c: any) => s + c.Returned, 0), color: "#dc2626" },
    { name: "Pending", value: barData.reduce((s: number, c: any) => s + c.Pending, 0), color: "#9ca3af" },
  ].filter((d) => d.value > 0);
  ...
  return (
    <div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={pieData} cx="50%" cy="50%" innerRadius={60} outerRadius={90}
                      paddingAngle={3} dataKey="value" label={({ name, percent }: any) => `${name} ${(percent * 100).toFixed(0)}%`}>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        ...
        <AcademicStructureOverview programs={programs ?? []} academicYears={academicYears ?? []} />
      </div>
